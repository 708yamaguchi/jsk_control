            ...
            if "Status" in line.strip():
                ...
                if i >= 10: # 少なくとも10行上まで存在するか確認
                    for j in range(i - 5, i):
                        ...
                        elif current_line.startswith("Pad_diameter"):
                            params["pad-diameter"] = float(current_line.split(" ")[1])
                            ...
                            if match:
                                ...
                                for point_str in point_strs:
                                    try:
                                        ...
                                        base_convex_hull_points.append((x, y))  # Ignore z because z is always 0
                                    except ValueError:
                                        print(f"Warning: Could not parse point '{point_str}'")
                        elif current_line.startswith("Collision"):
                            if "collision" not in params:
                                params["collision"] = []
                            params["collision"].append(current_line)
                # パースしたデータをprintする
                if params:
                    print("  Status:")
                    if "success" in line:
                    ...
                    if "collision" in params:
                        print("    " + "\n".join(params["collision"]))
                    else:
                        print("Collision is not found")
                    if "pad-diameter" in params:
                        print("  吸着パッドの直径[mm]")
                        print(f'    {params["pad-diameter"]}')
                    if base_convex_hull_points:
                        ...
                        if plot and "success" in line and area_cm2 < plot_thre:
                            plot_convex_hull_with_circle(
                                base_convex_hull_points,
                                circle_diameter=params["pad-diameter"],
